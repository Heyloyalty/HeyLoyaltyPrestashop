...
class HeyLoyaltyPrestashop extends Module
{

    public function __construct()
    {
        $this->name = 'HeyLoyaltyPrestashop';
        $this->tab = 'others';
        $this->version = '1.0.0';
        ...
        $this->displayName = $this->l('HeyLoyaltyPrestashop');
                        ...
                        $output .= '<div class="col-lg-3">';
                            $output .= '<select style="display:inline-block;width: 200px;" name="changeList" id="changeList">';
                                ...
                                foreach ($lists as $list) {
                                    $output .= '<option '. ((Configuration::get('Heyloyalty_list_id') == $list['id']) ? 'selected="selected"' : '') . ' value="' . $list['id'] . '">' . $list['name'] . '</option>';
                                }
                            $output .= '</select> ';
                        $output .= '</div>';
                    $output .= '</div>';
